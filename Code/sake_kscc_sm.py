...
using the Kernel density estimation approach and information theoretic based dissimilarity measure
Use simple matching measure for calculate the silhouette distance matrix.
'''
import shutil
import sys, os
...
def pairwise_distances(x):
    ...
    for row in range(n_samples):
        col = row + 1
        while col < n_samples:
            distance[row,col] = matching_dissim(x[row],x[col])
...
def run(argv):
    ...
    for n_clusters in range(2, 11):
        ...
        plt.suptitle(("#clusters = %d" % n_clusters),
                     fontsize=13)
        plt.savefig("{0}/kscc_sm_sake{1}.pdf".format(output_folder, n_clusters), dpi=300)
    # plt.show()
    import csv
