...
# import get_max_value_key, encode_features, get_unique_rows, decode_centroids
from lib import dissim as dis
...
def move_point_cat(point, ipoint, to_clust, from_clust, cl_attr_freq,
    ...
    for iattr, curattr in enumerate(point):
        ...
        if old_centroid_value == curattr:
            ...
            centroids[from_clust][iattr] = dis.get_max_value_key(from_attr_counts)
    ...
    for ik in range(n_clusters):
        for iattr in range(n_attrs):
            if sum(membship[ik]) == 0:
                ...
                centroids[ik, iattr] = dis.get_max_value_key(cl_attr_freq[ik][iattr])
...
def k_modes(X, n_clusters, max_iter, dissim, init, n_init, verbose, random_state, n_jobs):
    ...
    X, enc_map = dis.encode_features(X)
...
class KModes(BaseEstimator, ClusterMixin):
    ...
    def __init__(self, n_clusters=8, max_iter=100, cat_dissim=dis.matching_dissim,
    ...
    def predict(self, X, **kwargs):
        ...
        X = check_array(X, dtype=None)
        X, _ = dis.encode_features(X, enc_map=self._enc_map)
        return _labels_cost(X, self._enc_cluster_centroids, self.cat_dissim)[0]

    @property
    def cluster_centroids_(self):
        if hasattr(self, '_enc_cluster_centroids'):
            return dis.decode_centroids(self._enc_cluster_centroids, self._enc_map)
        else:
